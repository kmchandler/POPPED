  ...
  if (flicks.length <= 0) {
    ...
    <>
      ...
      <div className="d-flex flex-wrap cardContainer moodsCardsDiv">
        {flicks?.map((flix) => <FlickCard key={flix.flicksFirebaseKey} flickObj={flix} onUpdate={getMoodyFlicks} />)}
      </div>
    </>
  );
}
